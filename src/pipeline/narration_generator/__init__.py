...
class GoogleCloudTTSClient:
    ...
    async def synthesize(
        ...
        Chirp3-HD has a ~1000 char limit per request. For longer text,
        splits into sentence-boundary chunks and concatenates the MP3 bytes.
        Falls back to WaveNet-D (5000 char limit) if Chirp3-HD fails.
        """
        ...
        # Chirp3-HD limit is ~1000 chars — split on sentence boundaries
        _CHIRP_MAX = 900
    ...
    async def _synthesize_single(self, text: str, sample_rate: int) -> bytes:
        """Synthesize a single text chunk via Chirp3-HD → WaveNet-D fallback."""
        import base64 as _b64  # noqa: PLC0415
        import httpx  # noqa: PLC0415
        ...
        # Fallback: WaveNet-D on v1 (5000 char limit, always works)
        body_wavenet = {
            "input": {"text": text},
            "voice": {
                "languageCode": "en-US",
